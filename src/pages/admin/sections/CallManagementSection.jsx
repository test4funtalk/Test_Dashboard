...
const CallsTab = () => {
  ...
  const goToProfile = useCallback((id, role) => {
    if (!id) return;
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="border border-neutral-200 px-4 py-3">
                        <div
                          className={`flex items-center gap-2 ${call.callerId ? 'group cursor-pointer' : ''}`}
                          onClick={call.callerId ? (e) => { e.stopPropagation(); goToProfile(call.callerId._id, 'user'); } : undefined}
                          title={call.callerId ? 'View user profile' : undefined}
                        >
                          <div className={call.callerId ? 'flex-shrink-0 rounded-full transition group-hover:ring-2 group-hover:ring-neutral-900' : 'flex-shrink-0'}>
                            <AvatarDisplay src={call.callerId?.avatar || call.callerAvatar} name={call.callerId?.username || call.callerUsername} size="sm" />
                          </div>
                          <div className="min-w-0">
                            <p
                              className={`truncate font-medium ${call.status === 'ended' && (call.endedBy === 'user' || call.endedBy === 'system') ? 'text-red-600' : 'text-neutral-900'} ${call.callerId ? 'group-hover:underline' : ''}`}
                      ...
                      <td className="border border-neutral-200 px-4 py-3">
                        <div
                          className={`flex items-center gap-2 ${call.hostId ? 'group cursor-pointer' : ''}`}
                          onClick={call.hostId ? (e) => { e.stopPropagation(); goToProfile(call.hostId._id, 'host'); } : undefined}
                          title={call.hostId ? 'View host profile' : undefined}
                        >
                          <div className={call.hostId ? 'flex-shrink-0 rounded-full transition group-hover:ring-2 group-hover:ring-neutral-900' : 'flex-shrink-0'}>
                            <AvatarDisplay src={call.hostId?.avatar || call.hostAvatar} name={call.hostId?.username || call.hostUsername} size="sm" />
                          </div>
                          <div className="min-w-0">
                            <p
                              className={`truncate font-medium ${call.status === 'ended' && (call.endedBy === 'host' || call.endedBy === 'system') ? 'text-red-600' : 'text-neutral-900'} ${call.hostId ? 'group-hover:underline' : ''}`}
